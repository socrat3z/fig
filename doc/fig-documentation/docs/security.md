...
1. **Production Database Security** - Fig uses Sqllite out of the box but should be changed to a production RDBMS (SQL Server or PostgreSQL) for production deployments. All setting values are encrypted in the database but the database platform and access path must also be secured.
1. **PostgreSQL TLS** - When using PostgreSQL, prefer encrypted transport with certificate validation (for example `SSL Mode=VerifyFull` plus the appropriate root certificate) instead of plaintext or trust-only modes.
1. **Least-Privilege DB Accounts** - Use dedicated runtime database users with only required permissions. Avoid superuser/admin roles for the running Fig API.
